...
//    Classの先頭でプレイヤーの
//    横幅、高さ、ジャンプ中か落ちている最中かを表す状態、
//    ジャンプ時の速度、横方向への速度を定数として定義
    ...
    //ConstuructorでsetSizeMethodによる大きさ指定と、
    //mStateにPLAYER_STATE_FALLを設定
    init {
    ...
    //updateMethodでは表示する位置の決定と、状態が変わるかどうかの確認を行う
    fun update(delta: Float, accelX: Float) {
        ...
        // setXメソッド、setYメソッドを使うことで個別に設定することも可能
        setPosition(x + velocity.x * delta, y + velocity.y * delta)
        //継承元のGameObjectの更に親のClassであるSpriteClassで、あらかじめ用意されている
